...
/********************* Start Microassignment zone ********************/
	...
	void percolateDown(unsigned int hole)
	{
		...
		if (left <= _items.size() && _items[left] < _items[minIn])
			minIn = left;
		if (right <= _items.size() && _items[right] < _items[minIn])
			minIn = right;
		if (minIn != hole) {
			//swap min and hole
			int temp = _items[hole];
			_items[hole] = _items[minIn];
			_items[minIn] = _items[hole];

...
public:
	...
	void printLevelOrder() {
		...
		for (int current = 0; current <= h; current++) {
			printLevelOrderHelper(this->_root, current);
			cout << endl;
		}
	}

	void printLevelOrderHelper(Node<T>* root, int lvl) {
		if (root == nullptr || lvl == -1)
			return;
		if (lvl == 1)
			cout << root->value;
		else {
			printLevelOrderHelper(root->left, lvl - 1);
			printLevelOrderHelper(root->right, lvl - 1);
		}
	}
	 ...
	 */
	void printMaxPath() {
		vector<int> maxPath = printMaxPathHelper(this->_root);
		for (int i; i < maxPath.size(); i++) {
			cout << maxPath.at(i) << ' ';
		}
		cout << endl;
	}

	vector<int> printMaxPathHelper(Node<T>* root) {
		if (root == nullptr) {
			vector<int> empt = {};
			return empt;
		}

		vector<int> rightSide = printMaxPathHelper(root->right);
		vector<int> leftSide = printMaxPathHelper(root->left);

		if (rightSide.size() > leftSide.size()){

			rightSide.push_back(root->value);
			return rightSide;
		}
		else {
			leftSide.push_back(root->value);
			return leftSide;
		}
	}
	...
	bool contains(T value) {
		return containsHelp(this->_root, value);
